...
static wxString fpsGetFileExtension(const wxString &filename)
{
    ...
    for (; i != filename.size(); ++i)
        extension.Append(wxString(filename[i]));
    return extension;
}
...
static wxArrayString fpsGetOutputList(int rows, int cols, const wxString &inputFileName, int splitCount)
{
...
int fpsSplit(const wxString &inputFile, const wxString &outputDir, int rows, int cols)
{
    ...
    int splitCount = wxString::Format(_T("%d"), rows * cols).size();
